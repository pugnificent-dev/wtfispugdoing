...
| `/reconsider-replace` `number:` `image:` | Manually replace that numbered file (slash and right-click context menu share this path). Count stays the same. Unlocked numbers: edits the Edit per number post. **Locked numbers:** updates local file + Final + All Mario Images; does **not** repost to Edit per number (those posts are gone after `/lockin`). This is the only way to fill a locked gap. |
...
| `/snapshot-reactions` | Read-only dump of **unlocked** Edit per number and Reconsider reactions keyed by **image hash**. Locked numbers (no lineup post) are skipped so they do not trip the unmarked-snapshot warning. Rotates `reaction-snapshot-by-hash.json` / `reactions-by-hash.json` (current + one `.prev`). Does not compact, wipe, or change Discord. |
...
| --- | --- | --- |
| All Mario Images | snatch/lineup | Exactly `1.jpg`…`{count}.jpg` |
| Reconsider | Reconsider | Exactly the numbers currently in the Reconsider thread |
| Final | Final | Union of locked ranges: real images only (no PLACEHOLDER). Append-only on each `/lockin` (does not clear Final). Locked gaps stay absent from Final until `/reconsider-replace`. Unlocked numbers are not written while any lock exists. Without a lock: Edit per number `:check:` images via `/quality-controlled` |

`/lockin` upserts the new range into Final and **never clears Final** unless you pass `reset:true`. After each file is confirmed in Final, matching Edit per number posts are deleted. Local `output/{n}.jpg` files stay. Dry-run first. `/lockin unlock:true` clears all ranges and restores the old `:check:` workflow.
